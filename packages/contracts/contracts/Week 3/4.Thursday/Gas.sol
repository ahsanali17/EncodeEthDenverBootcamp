...
contract Constants {
    uint internal tradeFlag = 1;
    uint internal basicFlag = 0;
    uint internal dividendFlag = 1;
}

contract GasContract is Constants, Ownable {
    mapping(address => uint256) private balances;
    mapping(address => Payment[]) private payments;
    mapping(address => uint256) private whitelist;
    address[5] private administrators;
    uint32 private tradePercent = 12;
    address private contractOwner;
    uint private totalSupply = 0; // cannot be updated
    uint private paymentCounter = 0;
    uint private tradeMode = 0;
    ...
    function getTradingMode() private view returns (bool mode_) {
        bool mode = false;
